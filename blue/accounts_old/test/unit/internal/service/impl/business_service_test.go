...
func TestBusinessService_Create(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			svc := service.NewBusinessService(fakeTx{u: fUoW{w: wrepo}}, fakeQ{q: fQW{r: &fakeReadRepo{}}})
			id, err := svc.Create(context.Background(), &examplepb.CreateBusinessRequest{Name: "acme", GovernmentId: "RUC1"})
			if tc.wantErr {
				require.Error(t, err)
